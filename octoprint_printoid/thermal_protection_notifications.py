...
class ThermalProtectionNotifications(BaseNotification):
	...
	def set_thermal_protection(self, settings, maxTempDiff, bedShouldIncTemp, hotendShouldIncTemp,
							   chamberShouldIncTemp, delayBetweenNotif):
		if maxTempDiff >= 0:
			settings.set(["thermal_runway_threshold"], maxTempDiff)
			settings.set(["thermal_warmup_bed_seconds_threshold"], bedShouldIncTemp)
			settings.set(["thermal_warmup_hotend_seconds_threshold"], hotendShouldIncTemp)
			settings.set(["thermal_warmup_chamber_seconds_threshold"], chamberShouldIncTemp)
			settings.set(["thermal_threshold_minutes_frequency"], delayBetweenNotif)
			settings.save()
			return True
		else:
			return False
	...
	def __check_thermal_runway(self, temps, part, thermal_threshold, settings):
		thermal_threshold_minutes_frequency = settings.get_int(
		...
		if target_temp and target_temp > 0:
			...
			if actual_temp >= (target_temp + thermal_threshold):
				...
				if not self.__get_last_actual_temp(part) or self.__get_last_actual_temp(
						part) > actual_temp:
					if not self.__get_last_actual_temp(part):
						self._logger.debug(
							"Thermal runaway - Started to track {0}. Temp should go down. "
							"Actual {1} and Target {2} ".format(part, actual_temp, target_temp))
					else:
						self._logger.debug("Thermal runaway - Tracking {0}. Temp going down. "
										   "Actual {1} and Target {2} ".format(part, actual_temp,
																			   target_temp))
					...
					if self.__get_last_actual_temp_time(part) + cooldown_threshold > now:
						self._logger.debug(
							"Thermal runaway - Tracking {0}. Temp NOT going down. Will wait more time. "
							"Actual {1} and Target {2} ".format(part, actual_temp, target_temp))
						# We can still wait more time to let things cool down
						return
				elif self.__temp_increased_upto(actual_temp, self.__get_last_actual_temp(part),
												9) and \
						self.__get_last_actual_temp_time(part) + cooldown_threshold > now:
					self._logger.debug(
						"Thermal runaway - Tracking {0}. Temp went UP instead of DOWN. Will wait "
						"more time. Actual {1} and Target {2} ".format(part, actual_temp,
																	   target_temp))
					# We can still wait more time to let things cool down
					return
				...
				if actual_temp + below_target_threshold < target_temp:
					if not self.__get_last_actual_temp(part):
						self._logger.debug(
							"Thermal runaway - Started to track {0}. Temp should go up. "
							"Actual {1} and Target {2} ".format(part, actual_temp, target_temp))
					...
					if self._heater_timeout and actual_temp <= self.__get_last_actual_temp(
							part) + 1:
						...
						self._logger.debug(
							"Thermal runaway - Ignore checking since heater timed out waiting for user. "
							"Actual {1} and Target {2} ".format(part, actual_temp, target_temp))
						# Remember last temp so we can see if we temps are going up or not
						self.__save_last_temp(part, actual_temp)
						return
					...
					if (actual_temp == self.__get_last_actual_temp(part) or
						self.__temp_decreased_upto(actual_temp, self.__get_last_actual_temp(part),
												   2)) and \
							self.__get_last_actual_temp_time(part) + warmup_threshold > now:
						self._logger.debug(
							"Thermal runaway - Tracking {0}. Temp NOT going up. Will wait more time. "
							"Actual {1} and Target {2} ".format(part, actual_temp, target_temp))
					...
					if actual_temp <= self.__get_last_actual_temp(part):
						# Alert about possible thermal runaway (unless we just alerted)
						self.__thermal_runaway_detected(actual_temp, now, part, settings,
														target_temp,
														thermal_threshold_minutes_frequency)
					else:
	...
	def __thermal_runaway_detected(self, actual_temp, now, part, settings, target_temp,
		...
		if should_alert:
			self._logger.warning(
				"Possible thermal runaway detected for {0}. Actual {1} and Target {2} ".
