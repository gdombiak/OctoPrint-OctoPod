...
class BaseNotification:
	...
	def image(self, snapshot_url, hflip, vflip, rotate):
		...
		try:
			...
			# Besides this saves network bandwidth and Android device
	...
	def _send_base_notification(self, settings, include_image, event_code, event_param=None, silent_code_block=None):
		...
		for token in tokens:
			...
			last_result = self._alerts.send_alert_code(fcm_token, url, printer_id, printer_name,
													   event_code, image, event_param)

			if silent_code_block:
				# Send silent notification to refresh WearOS complication
				silent_code_block(fcm_token, image, printer_id, url)
